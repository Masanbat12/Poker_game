...
// Constants for determining whether hands are flushes or straights
#define FALSE 0
#define TRUE 1
...
int analyzeHand(int ranksinHand[], int suitsinHand[]) {
    int num_consec = 0;
    int i, rank, suit;
    int straight = FALSE, flush = FALSE, four = FALSE, three = FALSE, pairs = 0;
    ...
    rank = 0;
    while (ranksinHand[rank] == 0) rank++;
    for (; rank < 13 && ranksinHand[rank]; rank++) num_consec++;
    if (num_consec == 5) {
        straight = TRUE;
    }
